        ...
        **kwargs
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                print(f"⚠️ Google Search timeout after {self.timeout}s (attempt {attempt + 1}/{self.max_retries})")
                print(f"   Query: '{query}'")
                if attempt == self.max_retries - 1:
                    print(f"   All {self.max_retries} retries exhausted")
                    return []
    ...
    async def get_page_content(self, url: str) -> str:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                print(f"❌ Scrape HTTP error: {e.response.status_code} - {e.response.reason_phrase}")
                print(f"   URL: {url}")
                if attempt == self.max_retries - 1:
                    print(f"   All {self.max_retries} retries exhausted")
                    return ""
                ...
                print(f"⚠️ Scrape timeout after {self.timeout}s (attempt {attempt + 1}/{self.max_retries})")
                print(f"   URL: {url}")
                if attempt == self.max_retries - 1:
                    print(f"   All {self.max_retries} retries exhausted")
                    return ""
                ...
                error_type = type(e).__name__
                print(f"⚠️ Scrape error for {url} (attempt {attempt + 1}/{self.max_retries}): {error_type}")
                print(f"   Error: {str(e)}")
                if attempt == self.max_retries - 1:
                    print(f"   All {self.max_retries} retries exhausted")
                    return ""
